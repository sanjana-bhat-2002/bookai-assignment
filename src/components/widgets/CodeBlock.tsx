...
import  { monoBlue, atomOneDarkReasonable } from 'react-syntax-highlighter/dist/esm/styles/hljs';
...
const CodeBlock = ({ codeString, language, isOutput = false }: CodeBlockProps) => {
  const { themeMode } = useTheme()
  
  const [codeBlockTheme, setCodeBlockTheme] = useState<ThemeType>(lightTheme);
  
  useEffect(() => {
    themeMode === "dark" ? setCodeBlockTheme(atomOneDarkReasonable) : setCodeBlockTheme(lightTheme);
  }, [themeMode]);
  ...
  return (
    ...
    <SyntaxHighlighter className="rounded-b-lg w-full" language={language} style={codeBlockTheme} wrapLongLines={true} wrapLines={true}>
      {codeString}
    </SyntaxHighlighter>
    </div>
